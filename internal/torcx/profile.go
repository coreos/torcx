...
func AddToProfile(profilePath string, im Image) error {
	...
	// Try v1 first
	v1Profile, err := getProfileV1(profilePath)
	if err == nil {
		return addToProfileV1(profilePath, st.Mode().Perm(), v1Profile, &im)
	}

	// Fallback to v0
	v0Profile, err := getProfileV0(profilePath)
	if err == nil {
...
// getProfileV0 reads a profile from the given path, returning the unmarshalled
// JSON (v0) profile manifest.
...
// getProfileV1 reads a profile from the given path, returning the unmarshalled
// JSON (v1) profile manifest.
...
// addToProfileV0 adds an image to the given JSON (v0) profile manifest,
// and writes it to disk at the given path.
...
// addToProfileV1 adds an image to the given JSON (v1) profile manifest,
// and writes it to disk at the given path.
